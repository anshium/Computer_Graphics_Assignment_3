...
mkdir -p output_scenes

reportfile="./output_scenes/report.md"
...
runQ3(){
	mkdir -p ./output_scenes/Question_3/$3

	echo "### Method: \`$3\` | SPP: \`$1\` | Anti-Aliasing: \`On\`" >> $reportfile

	echo "Small:"
	./build/render ./scenes/Assignment\ 3/Question\ 3/small.json ./output_scenes/Question_3/$3/small_$1_spp.png $1 $2 | tee temp_output_file.txt
	echo "- Small: " \`$(cat temp_output_file.txt | grep "Render" | awk '{print $((NF - 1)), "ms"}')\` >> $reportfile

	echo "Med:"
	./build/render ./scenes/Assignment\ 3/Question\ 3/med.json ./output_scenes/Question_3/$3/med_$1_spp.png $1 $2  | tee temp_output_file.txt
	echo "- Med: " \`$(cat temp_output_file.txt | grep "Render" | awk '{print $((NF - 1)), "ms"}')\` >> $reportfile
	
	echo "Big:"
	./build/render ./scenes/Assignment\ 3/Question\ 3/big.json ./output_scenes/Question_3/$3/big_$1_spp.png $1 $2 | tee temp_output_file.txt
	echo "- Big: " \`$(cat temp_output_file.txt | grep "Render" | awk '{print $((NF - 1)), "ms"}')\` >> $reportfile
	
	echo "Many:"
	./build/render ./scenes/Assignment\ 3/Question\ 3/many.json ./output_scenes/Question_3/$3/many_$1_spp.png $1 $2 | tee temp_output_file.txt
	echo "- Many: " \`$(cat temp_output_file.txt | grep "Render" | awk '{print $((NF - 1)), "ms"}')\` >> $reportfile

	echo
}

for spp in 10 100 1000;
do
