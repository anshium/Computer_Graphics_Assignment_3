...
Ray Camera::generateRay(int x, int y) {
    Vector3f pixelPoint = this->upperLeft + (next_float() * this->pixelDeltaU + next_float() * this->pixelDeltaV);
    pixelPoint = pixelPoint + x * this->pixelDeltaU + y * this->pixelDeltaV;

    Vector3f direction = Normalize(pixelPoint - this->from);
